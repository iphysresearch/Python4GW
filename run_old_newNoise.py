...
for snr in list([1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]):
    ...
    train_dict['%s' %int(snr*10)] = pd.concat([data_train, pd.DataFrame(noise_train, columns=colomns[:8192]).iloc[:data_train.shape[0],:]])[colomns]
    test_dict['%s' %int(snr*10)] = pd.concat([data_test, pd.DataFrame(noise_test, columns=colomns[:8192]).iloc[:data_test.shape[0],:]])[colomns]
    ...
    Info = Train(train = train_dict['%s' %int(snr*10)]
                 ...
                 , batch_size=256, lr=0.0003, epochs=20
    ...
    params_ = nd.load('./SNR%s_OURs/params_%s'  %( int(snr*10) ,best_params_epoch))
...
for SNR_ in SNR_list:  # 不同的模型参数
    auc_ = []
    address = './SNR%s_%s/' %(int(SNR_*10),model_name)
    # address = '/floyd/input/params/SNR%s_%s/' %(int(SNR_*10),model_name)
...
# -m "AUCOURs_old_newNoise" \
# "bash setup_floydhub.sh && python run_old_newNoise.py"
...
# --data wctttty/datasets/ligo_localnoise_9_9000_8192/1:ligo_localnoise_9_9000_8192_1 \
# --data wctttty/datasets/ligo_localnoise_9_9000_8192/2:ligo_localnoise_9_9000_8192_2 \
# --data wctttty/projects/python4gw/299:params \
# -m "AUCOURs_old_newNoise" \
# "bash setup_floydhub.sh && python run_old_newNoise.py"
