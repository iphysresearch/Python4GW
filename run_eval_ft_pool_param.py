...
params_adds['pool_params'] = params_adds[0].map(lambda x: int(x.split('_')[2]))
params_adds = params_adds.sort_values('pool_params', ascending=True)[0].values.tolist()
...
for param_add, hyperparam in zip(params_adds, Fine_tune('pool_type_kernel', [i for i in zip(pool_type_tuple, values)])):
    ...
    OURS_ori = ConvNet(conv_params = {'kernel': ((1,16), (1,8), (1,8)), 
                       ...
                       act_params = act_type,
                       pool_params = {'pool_type': (pool_type, pool_type, pool_type),
                                        'kernel': pool_kernel,
                                        'stride': ((1,2), (1,2), (1,2),),
                                        'padding': ((0,0),(0,0), (0,0),),
                                        'dilate': ((1,1), (1,1), (1,1),)},
