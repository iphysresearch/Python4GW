...
def init_params(num_fc1, num_fc2, num_outputs, sl):
    ...
    W1 = nd.random_normal(loc=0, scale=weight_scale, shape=(64, 1, 1, 16), ctx=ctx )
    # b1 = nd.zeros(shape=16, ctx=ctx)
    
    # P2 = np.sqrt(6)/np.sqrt(16)
    # W2 = nd.random_uniform(low = -P2, high= P2, shape=(32, 16, 1, 8), ctx=ctx)
    W2 = nd.random_normal(loc=0, scale=weight_scale, shape=(128, 64, 1, 16), ctx=ctx )
    # b2 = nd.zeros(shape=32, ctx=ctx)
    
    # P3 = np.sqrt(6)/np.sqrt(32)
    # W3 = nd.random_uniform(low = -P3, high= P3, shape=(64, 32, 1, 8), ctx=ctx)
    W3 = nd.random_normal(loc=0, scale=weight_scale, shape=(256, 128, 1, 16), ctx=ctx )
    # b3 = nd.zeros(shape=64, ctx=ctx)
    
    # P3 = np.sqrt(6)/np.sqrt(32)
    # W3 = nd.random_uniform(low = -P3, high= P3, shape=(64, 32, 1, 8), ctx=ctx)
    W4 = nd.random_normal(loc=0, scale=weight_scale, shape=(512, 256, 1, 32), ctx=ctx )
    # b3 = nd.zeros(shape=64, ctx=ctx)
...
#     print(W4)
    ...
    b1 = nd.random_normal(shape=64, scale=weight_scale, ctx=ctx)
...
#     W3 = nd.random_normal(shape=(64, 32, 1, 8), scale=weight_scale, ctx=ctx)
    b3 = nd.random_normal(shape=256, scale=weight_scale, ctx=ctx)
    b4 = nd.random_normal(shape=512, scale=weight_scale, ctx=ctx)    
...
def evaluate_accuracy(data_iterator, num_examples, batch_size, params, net, pool_type,pool_size,pool_stride):
    ...
    for batch_i, (data, label) in enumerate(data_iterator):
        ...
        output, _ = net_PLB(data, params,pool_type=pool_type,pool_size = pool_size,pool_stride=pool_stride)
...
#         # random fc layers
        ...
        params, vs, sqrs = init_params(num_fc1 = 128, num_fc2 = 64, num_outputs = 2, sl=sl)
    ...
    if Debug and (not params_init):
        ...
        for data, _ in train_data:
            data = data.as_in_context(ctx).reshape((batch_size,1,1,-1))
            break
        _, _ = net_PLB(data, params, debug=Debug, pool_type=pool_type,pool_size = pool_size,pool_stride=pool_stride)
        print()
...
#           Increment t before invoking adam.
            ...
            output_v, _ = net_PLB(data_v, params, pool_type=pool_type,pool_size = pool_size,pool_stride=pool_stride)
...
#                 print('Curr_loss: ', curr_loss)
        ...
        test_accuracy = evaluate_accuracy(test_data, num_examples, batch_size, params, net_PLB,pool_type=pool_type,pool_size = pool_size,pool_stride=pool_stride)
        train_accuracy = evaluate_accuracy(train_data, num_examples, batch_size, params, net_PLB,pool_type=pool_type,pool_size = pool_size,pool_stride=pool_stride)
...
# print(os.listdir('/floyd/input/params/'))
...
for snr in SNR_list:
    ...
    try: 
        ...
        train_dict['%s' %int(snr*100)] = pd.concat([data_train, pd.DataFrame(noise_train, columns=colomns[:8192]).iloc[:data_train.shape[0],:]])[colomns]
    ...
    address = 'SNR%s_PLB' %int(snr*100)
    mkdir_checkdir(path = "./%s" %address)



    print('Start Training at SNR = %s ...' %int(snr*100))
    
    Info = Train(train = train_dict['%s' %int(snr*100)]
                 ,test = test_dict['%s' %int(snr*100)], Debug=True , params_init = params_
                 , batch_size=256, lr=0.0001, epochs=20
                 , smoothing_constant = .01, SNR = 1
                 , sl=sl, pool_type=pool_type ,pool_size = pool_size, pool_stride=pool_stride
                 , num_fc1 = 128, num_fc2 = 64, num_outputs = 2, period = 256)
    ...
    try:
        ...
        print(e)
        print('Early stoping with best_params_epoch=%s' %str(best_params_epoch))
    finally:    
        params_ = nd.load('./SNR%s_PLB/params_%s' %( int(snr*100) ,best_params_epoch))
...
for SNR_ in SNR_list:  # 不同的模型参数
    ...
    # address = '/floyd/input/params/SNR%s_%s/' %(int(SNR_*100),model_name)
    ...
    print('Finished!', end='')
    sys.stdout.write("\r")
np.save('AUC_%s_old_newNoise' %(model_name), AUC)
...
# -m "AUCPLB_old_newNoise" \
# "bash setup_floydhub.sh && python run_old_PLB_newNoise.py"
...
# --data wctttty/datasets/ligo_localnoise_9_9000_8192/1:ligo_localnoise_9_9000_8192_1 \
# --data wctttty/datasets/ligo_localnoise_9_9000_8192/2:ligo_localnoise_9_9000_8192_2 \
# --data wctttty/projects/python4gw/295:params \
# -m "AUCPLB_old_newNoise" \
# "bash setup_floydhub.sh && python run_old_PLB_newNoise.py"
